...
/** Per-session usage counters returned by image search and voice endpoints. */
export interface SessionUsage {
  image_search_count: number;
  image_search_limit: number;
  voice_message_count: number;
  voice_message_limit: number;
}
...
/** Payload portion of an /image-search success response (chat-compatible). */
export interface ImageSearchData extends ChatData {
  match_type?: "exact" | "broad" | "unknown" | "image_clarification" | string;
...
export interface ChatMessage {
  ...
  documentName?: string;
  /** True while progressive text reveal is running for this bubble. */
  isGenerating?: boolean;
  /** Frontend-only partial text during generation; never sent to backend. */
  displayedText?: string;
  /** When true, render final Markdown formatting (after generation completes). */
  generationComplete?: boolean;
  /** Inline machine comparison table + verdicts (chat comparison mode). */
