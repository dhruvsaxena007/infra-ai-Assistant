...
exports.AzureAssistantProvider = exports.AzureChatCompletionProvider = exports.AzureCompletionProvider = exports.AzureEmbeddingProvider = exports.AzureGenericProvider = exports.DEFAULT_AZURE_API_VERSION = void 0;
...
function calculateAzureCost(modelName, config, promptTokens, completionTokens) {
    return (0, shared_1.calculateCost)(modelName, { cost: undefined }, promptTokens, completionTokens, AZURE_MODELS);
}
...
class AzureEmbeddingProvider extends AzureGenericProvider {
    async callEmbeddingApi(text) {
        ...
        try {
            ({ data, cached } = (await (0, cache_1.fetchWithCache)(`${this.getApiBaseUrl()}/openai/deployments/${this.deploymentName}/embeddings?api-version=${this.config.apiVersion || exports.DEFAULT_AZURE_API_VERSION}`, {
                method: 'POST',
                headers: {
...
class AzureCompletionProvider extends AzureGenericProvider {
    async callApi(prompt, context, callApiOptions) {
        await this.ensureInitialized();
        (0, invariant_1.default)(this.authHeaders, 'auth headers are not initialized');
        if (!this.getApiBaseUrl()) {
        ...
        let data, cached = false;
        try {
            ({ data, cached } = (await (0, cache_1.fetchWithCache)(`${this.getApiBaseUrl()}/openai/deployments/${this.deploymentName}/completions?api-version=${this.config.apiVersion || exports.DEFAULT_AZURE_API_VERSION}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    ...this.authHeaders,
...
class AzureChatCompletionProvider extends AzureGenericProvider {
        ...
        const body = {
            ...
            ...(config.o1
                ? {
                    ...
                    reasoning_effort: (0, util_1.renderVarsInObject)(config.reasoning_effort, context?.vars),
                }
                : {
                    max_tokens: config.max_tokens ?? (0, envars_1.getEnvInt)('OPENAI_MAX_TOKENS', 1024),
    ...
    async callApi(prompt, context, callApiOptions) {
        ...
        try {
            const url = config.dataSources
                ? `${this.getApiBaseUrl()}/openai/deployments/${this.deploymentName}/extensions/chat/completions?api-version=${config.apiVersion || exports.DEFAULT_AZURE_API_VERSION}`
                : `${this.getApiBaseUrl()}/openai/deployments/${this.deploymentName}/chat/completions?api-version=${config.apiVersion || exports.DEFAULT_AZURE_API_VERSION}`;
...
class AzureAssistantProvider extends AzureGenericProvider {
    constructor(deploymentName, options = {}) {
    ...
    async callApi(prompt, context, callApiOptions) {
        await this.ensureInitialized();
        (0, invariant_1.default)(this.assistantsClient, 'Assistants client not initialized');
        if (!this.getApiBaseUrl()) {
            throwConfigurationError('Azure API host must be set.');
        }
        ...
        while (run.status === 'in_progress' ||
            ...
            if (run.status === 'requires_action') {
                ...
                (0, invariant_1.default)(requiredAction, 'Run requires action but no action is provided');
