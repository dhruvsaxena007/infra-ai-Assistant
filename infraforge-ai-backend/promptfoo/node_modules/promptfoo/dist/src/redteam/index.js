    ...
    if (strategy.id === 'multilingual') {
        return (totalPluginTests *
            (Math.max(strategies.length, 1) *
                (Object.keys(strategy.config?.languages ?? {}).length || multilingual_1.DEFAULT_LANGUAGES.length)));
    }
...
function calculateTotalTests(plugins, strategies) {
    ...
    const numLanguages = multilingualStrategy
        ? Object.keys(multilingualStrategy?.config?.languages ?? {}).length || multilingual_1.DEFAULT_LANGUAGES.length
        : 1;
    ...
    const totalTests = strategies.length === 0
        ? totalPluginTests
 ...
 */
async function synthesize({ entities: entitiesOverride, injectVar, language, maxConcurrency = 1, plugins, prompts, provider, purpose: purposeOverride, strategies, delay, abortSignal, }) {
    ...
    const showProgressBar = !isWebUI && process.env.LOG_LEVEL !== 'debug' && (0, logger_1.getLogLevel)() !== 'debug';
    if (showProgressBar) {
    ...
    checkAbort();
    const { testCases: strategyTestCases, strategyResults } = await applyStrategies(pluginTestCases, strategies.filter((s) => s.id !== 'basic' && s.id !== 'multilingual'), injectVar);
