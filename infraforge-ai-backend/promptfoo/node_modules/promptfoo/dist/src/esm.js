...
async function importModule(modulePath, functionName) {
    logger_1.default.debug(`Attempting to import module: ${JSON.stringify({ resolvedPath: path.resolve(modulePath), moduleId: modulePath })}`);
    try {
        ...
        try {
            ...
            const mod = importedModule?.default?.default || importedModule?.default || importedModule;
            logger_1.default.debug(`Successfully required module: ${JSON.stringify({ resolvedPath: path.resolve(modulePath), moduleId: modulePath })}`);
            if (functionName) {
                logger_1.default.debug(`Returning named export: ${functionName}`);
                return mod[functionName];
            }
