...
function parseRawRequest(input) {
    ...
    try {
        const messageModel = http_z_1.default.parse(adjusted);
        return {
...
class HttpProvider {
    ...
    async callApi(prompt, context) {
        ...
        logger_1.default.debug(`[HTTP Provider]: Transformed prompt: ${transformedPrompt}. Original prompt: ${prompt}`);
        ...
        logger_1.default.debug(`[HTTP Provider]: Response: ${response.data}`);
        ...
        logger_1.default.debug(`[HTTP Provider]: Response (HTTP ${response.status}): ${response.data}`);
        const ret = {};
        if (context?.debug) {
    ...
    async callApiWithRawRequest(vars) {
        ...
        logger_1.default.debug(`[HTTP Provider]: Calling ${url} with raw request: ${parsedRequest}`);
        const response = await (0, cache_1.fetchWithCache)(url, {
            method: parsedRequest.method,
            headers: parsedRequest.headers,
            ...(parsedRequest.body && { body: parsedRequest.body.text.trim() }),
        }, shared_1.REQUEST_TIMEOUT_MS, 'text', undefined, this.config.maxRetries);
        logger_1.default.debug(`[HTTP Provider]: Response: ${response.data}`);
        if (!(await this.validateStatus)(response.status)) {
            throw new Error(`HTTP call failed with status ${response.status} ${response.statusText}: ${response.data}`);
        }
