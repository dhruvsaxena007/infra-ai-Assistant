    ...
    async callEmbeddingApi(text) {
        try {
            ...
            if (this.providerName === 'openai') {
                ...
                const parentClass = new openai_2.OpenAiEmbeddingProvider(this.modelName, this.providerOptions);
                ...
                const parentClass = new azure_2.AzureEmbeddingProvider(this.modelName, this.providerOptions);
                gatewayEmbeddingModel = provider.embeddingModel({
    ...
    async callApi(prompt, context, callApiOptions) {
        ...
        try {
            ...
            if (formatType === 'openai') {
                ...
                const openAiProvider = new openai_2.OpenAiChatCompletionProvider(this.modelName, this.providerOptions);
            ...
            if (this.providerName === 'openai') {
                const provider = new openai_1.OpenAI();
                const parentClass = new openai_2.OpenAiChatCompletionProvider(this.modelName, this.providerOptions);
                const apiKey = parentClass.getApiKey();
                if (!apiKey) {
                    throw new Error('OpenAI API key is not set. Set the OPENAI_API_KEY environment variable or add `apiKey` to the provider config.');
                }
                ...
                const provider = new azure_1.Azure();
                const parentClass = new azure_2.AzureChatCompletionProvider(this.modelName, this.providerOptions);
                ...
                const provider = new anthropic_1.Anthropic();
                const parentClass = new anthropic_2.AnthropicMessagesProvider(this.modelName, this.providerOptions);
                const apiKey = parentClass.getApiKey();
                if (!apiKey) {
        ...
        try {
            ...
            if (this.providerName === 'openai') {
                cost = (0, openai_2.calculateOpenAICost)(this.modelName, costConfig, response.response.usage?.promptTokens, response.response.usage?.completionTokens);
            }
            else if (this.providerName === 'azureopenai') {
                cost = (0, azure_2.calculateAzureCost)(this.modelName, {}, response.response.usage?.promptTokens, response.response.usage?.completionTokens);
            }
            else if (this.providerName === 'anthropic') {
                cost = (0, anthropic_2.calculateAnthropicCost)(this.modelName, costConfig, response.response.usage?.promptTokens, response.response.usage?.completionTokens);
            }
