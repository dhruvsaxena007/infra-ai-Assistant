"""
Search filter merge engine — session memory, follow-ups, clarification answers.

Does NOT classify support/refund/payment/greeting/out-of-scope intents.
That responsibility belongs to intent_classifier.py + assistant_router.py.
...
FOLLOW_UP_PHRASES = (
    ...
    "upto", "up to", "kam", "sasta", "premium", "expensive", "higher budget",
    "rent only", "buy only", "nearby",
)
...
def _is_true_follow_up(
    ...
    *,
    ...
    # Full new search (category + city) overwrites session unless explicit "same/what about"
    if (
        parsed.get("category")
        and parsed.get("city")
        and not _has_same_phrase(message)
    ):
        return False
    ...
    # City switch refinement: "same in delhi" keeps category, updates city.
    if parsed.get("city") and not parsed.get("category"):
        if _has_same_phrase(message) and (last.get("category") or last.get("brand")):
            return True
...
def _merge_filters(
    ...
    *,
    ...
    # Purpose revision overrides stale category on follow-ups
    purpose_key = new.get("purpose_key")
    if purpose_key:
        from app.ai.purpose_taxonomy import category_compatible_with_purpose, primary_category_for_purpose

        derived = primary_category_for_purpose(purpose_key)
        if derived:
            old_cat = merged.get("category")
            if not old_cat or not category_compatible_with_purpose(old_cat, purpose_key):
                merged["category"] = new.get("category") or derived
...
def _apply_higher_budget_adjustment(filters: dict, message: str) -> dict:
    from app.ai.intent_signals import is_higher_budget_signal

    if not is_higher_budget_signal(message, {"last_filters": filters}):
        return filters
    filters = dict(filters)
    if filters.get("max_price"):
        filters["max_price"] = int(filters["max_price"] * 1.35)
    else:
        filters["min_price_hint"] = True
    return filters
...
async def resolve_user_intent(
    ...
    *,
    ...
    """
    Merge search filters for a turn already approved for machine search by the router.

    Returns filter metadata ready for execute_machine_search_turn.
    """
    result = _base_intent()
    message = (message or "").strip()
    history_text = get_conversation_history_as_text(session_id)
    has_history = bool(history_text)
    _ = recommendation_context  # reserved — project recommendation handled in router
    _ = greeted
    _ = router_intent
    ...
    resolved_message, used_image = resolve_message_with_image_context(
        session_id, message, router_intent=router_intent,
    )
    ...
    try:
        ...
        conv = get_current_state() or load_conversation_state(session_id)
        collected = (conv or {}).get("collected_fields") or {}
    except Exception:
        collected = {}
    ...
    if pending and is_clarification_answer(message, pending, parsed=parsed):
        ...
        if pending.get("missing_field") == "machine_purpose":
            purpose_key = merged.get("purpose_key")
            city = merged.get("city")
            if purpose_key and not city:
                new_pending = {
                    **pending,
                ...
                result["clear_pending_clarification"] = False
                result["save_pending_clarification"] = new_pending
                result["intent"] = "clarification"
                result["assistant_mode"] = "clarification"
                result["pending_clarification"] = new_pending
            elif city and not purpose_key:
                new_pending = {**pending, "city": city, "missing": ["purpose"]}
                result["clear_pending_clarification"] = False
                result["save_pending_clarification"] = new_pending
                result["intent"] = "clarification"
                result["assistant_mode"] = "clarification"
    ...
    if router_intent in ("higher_budget_query", "cheaper_option_query"):
        is_fup = bool(last_filters.get("category") or last_filters.get("city"))
    from app.ai.search_refinement_engine import detect_refinement_type

    if detect_refinement_type(message, parsed) and (last_filters.get("category") or last_filters.get("city")):
        is_fup = True
    if used_image and parsed.get("category"):
        is_fup = False
    ...
    merged = _apply_cheaper_adjustment(merged, message)
    merged = _apply_higher_budget_adjustment(merged, message)
