...
from app.ai.assistant_response_style import (
    ...
    project_recommendation_explanation,
    search_followup_suggestions,
)
...
def reset_session_state(session_id: str) -> None:
    """Clear persisted and in-memory assistant session memory."""
    from app.chatbot.image_context_memory import clear_image_context
    from app.chatbot.session_usage_limits import reset_session_usage
    from app.database.persistent_store import persist_session_fields
    ...
    reset_session_usage(session_id)
...
async def _run_search_no_result_recovery(
    database,
    *,
    session_id: str,
    intent_name: str,
    ...
    """Invoke Phase 8 recovery engine for machine search no-results."""
    from app.ai.assistant_debug_trace import record_recovery
    from app.ai.conversation_state_manager import get_current_state, load_conversation_state
    from app.ai.no_result_recovery import run_no_result_recovery

    conv_state = get_current_state() or load_conversation_state(session_id)
    permission_decision = (classification or {}).get("action_decision") or {
    ...
    recovery = await run_no_result_recovery(
        database,
        intent=intent_name or "machine_search",
        ...
        tool_result={"fallback_reason": fallback_reason or "no_exact_match"},
        conversation_state=conv_state,
        permission_decision=permission_decision,
        purpose_key=purpose_key,
        lang=lang,
    )
    record_recovery(recovery, recovery_state_saved=False)
    return recovery
...
async def chatbot_response(session_id: str, user_message: str, database, *, selected_machine_id: str | None = None):
    ...
    from app.ai.conversation_state_manager import begin_turn, end_turn, get_current_state
    from app.ai.user_memory_profile import (
        apply_profile_hints_to_session_context,
        begin_profile_turn,
        end_profile_turn,
    )
    ...
    if selected_machine_id:
        ...
        from app.utils.machine_repository import fetch_machine_by_id

        machines = (session_ctx.get("last_results") or {}).get("last_machines") or []
        selected = next(
            (m for m in machines if str(m.get("id") or m.get("_id") or "") == selected_machine_id),
            None,
        )
        if not selected or not selected.get("name"):
            fetched = await fetch_machine_by_id(database, selected_machine_id)
            selected = slim_machine(fetched) if fetched else {"id": selected_machine_id}
        session_ctx = {**session_ctx, "selected_machine": selected}
        _save_session_context(session_id, session_ctx)
    user_id = session_ctx.get("user_id")
    profile, profile_hints, profile_token = begin_profile_turn(session_id, user_id=user_id)

    if profile_hints:
        _save_session_context(
            session_id,
            apply_profile_hints_to_session_context(session_ctx, profile_hints),
        )

    trace_token = None
    conv_state = begin_turn(session_id)
    conv_state["user_profile"] = profile
    conv_state["user_profile_hints"] = profile_hints

    if assistant_debug_enabled():
        trace_token = begin_chat_trace(session_id, user_message)
        record_state_before(conv_state)
        record_user_profile({
            "loaded": True,
            "profile_source": (profile.get("privacy") or {}).get("source", "none"),
            "profile_hints_used": list(profile_hints.keys()),
            "profile_updated": False,
            "updated_fields": [],
            "sensitive_fields_skipped": True,
        })
        memory_before = build_memory_snapshot(
            _get_session_context(session_id),
            _get_last_filters(session_id),
            _get_pending_clarification(session_id),
        )
        from app.ai.assistant_debug_trace import record_memory_before
        record_memory_before(memory_before)
    try:
        resp = await handle_assistant_message(session_id, user_message, database)
    except Exception as exc:
        import traceback
        print(f"[chatbot] handle_assistant_message failed: {exc}")
        traceback.print_exc()
        from app.ai.safe_error_fallback import build_safe_error_response
        from app.chatbot.language import detect_query_language

        resp = build_safe_error_response(
            stage="assistant_router",
            exc=exc,
            user_message=user_message,
            lang=detect_query_language(user_message),
        )
    try:
        await log_turn_from_response(database, session_id, user_message, resp)
    except Exception as exc:
        print(f"[turn_analytics] skip: {exc}")

    state = get_current_state() or conv_state
    ...
    end_turn(
        ...
        action_decision=state.get("_last_action_decision"),
    )

    from app.ai.user_memory_profile import get_profile_meta
    end_profile_turn(
        profile,
        user_message=user_message,
        response=resp,
        ...
        intent=ctx.get("intent"),
        reply_lang=ctx.get("reply_language"),
        profile_token=profile_token,
    )

    if assistant_debug_enabled():
        record_state_after(state, saved=True)
        memory_after = build_memory_snapshot(
            _get_session_context(session_id),
            _get_last_filters(session_id),
            _get_pending_clarification(session_id),
        )
        finalize_chat_trace(resp, session_id, memory_after=memory_after)
        end_chat_trace(trace_token)
...
async def execute_machine_search_turn(
    ...
    """Machine search execution — only called when router sets should_search_machines."""
    ...
    # Hard gate — router must approve search; safety net if mis-routed
    if classification and not classification.get("should_search_machines") and not forced_filters:
        resp = build_response(
            classification.get("intent") or "unknown",
            entities=(classification.get("entities") or {}),
            lang=reply_lang,
        )
        save_conversation(session_id, user_message, resp["message"])
        return success_response(
            message=resp["message"],
            data=_assistant_payload(
                message=resp["message"],
                machines=[],
                assistant_mode=resp.get("assistant_mode", "unknown"),
                suggestions=resp.get("suggestions") or [],
                reply_language=reply_lang,
                context_extra={"intent": classification.get("intent"), "search_gate": "blocked"},
                input_meta=input_meta,
            ),
        )

    if forced_filters:
        last = _get_last_filters(session_id)
        # Forced filters from requirement engine win — do not let stale category bleed in
        merged_force = {**_DEFAULT_FILTERS, **last, **forced_filters}
        for key in ("category", "city", "purpose_key", "max_price", "listing_type"):
            if forced_filters.get(key) is not None:
                merged_force[key] = forced_filters[key]
    ...
    intent = await resolve_user_intent(
        ...
        greeted=greeted,
        router_intent=(classification or {}).get("intent"),
    )

    if search_flags:
        if search_flags.get("list_all"):
            intent["list_all"] = True
        if search_flags.get("recommended_categories"):
            intent["recommended_categories"] = search_flags["recommended_categories"]
            intent["intent"] = "project_recommendation"
            intent["assistant_mode"] = "project_recommendation"
        if search_flags.get("project_type"):
            intent["project_type"] = search_flags["project_type"]
    ...
    if intent.get("intent") == "clarification":
        ...
        missing = (clarify.get("missing_field") if clarify else "filters") or "filters"
        if clarify and clarify.get("missing_field") == "machine_purpose":
            from app.chatbot.assistant_intelligence import (
                broad_machine_clarification_message,
                machine_purpose_city_message,
            )

            pk = clarify.get("purpose_key")
            if pk and clarify.get("missing") == ["city"]:
                reply = machine_purpose_city_message(pk, lang=reply_lang)
            else:
                reply = broad_machine_clarification_message(lang=reply_lang)
            suggestions = clarification_suggestions("machine_purpose", city=clarify.get("city"))
        elif clarify and clarify.get("missing_field") == "category" and clarify.get("city"):
            ...
            cat_hint = clarify.get("category") if clarify else merged.get("category")
            if cat_hint and not (clarify or {}).get("city") and missing in ("city", "filters"):
                from app.ai.user_memory_profile import profile_aware_clarification_message

                prof_reply = profile_aware_clarification_message(
                    lang=reply_lang,
                    category=cat_hint,
                    profile_hints=_get_session_context(session_id).get("user_profile_hints"),
                )
                if prof_reply:
                    reply = prof_reply
            if not suggestions:
    ...
    recovery_output: dict | None = None
    ...
    if requested_category:
        ...
        if exact_raw:
            ...
            # When a city was specified, stay in the SAME city — never cross-city dump.
                ...
                if not exact_match_found and not alternatives_raw:
                    from app.ai.no_result_recovery import (
                        build_recovery_draft_message,
                        recovery_context_for_state,
                    )

                    recovery_output = await _run_search_no_result_recovery(
                        database,
                        session_id=session_id,
                        intent_name=intent.get("intent") or "machine_search",
                        merged=merged,
                        requested_category=requested_category,
                        city=city,
                        max_price=max_price,
                        listing_type=listing_type,
                        fallback_reason=fallback_reason or "no_match_in_city",
                        classification=classification,
                        purpose_key=purpose_key,
                        lang=reply_lang,
                    )
                    auto = recovery_output.get("auto_retry_results") or {}
                    if recovery_output.get("auto_retried") and auto.get("machines"):
                        alternatives_raw = auto["machines"]
                        matched_category = (
                            alternatives_raw[0].get("category")
                            if alternatives_raw else None
                        )
                        fallback_used = True
                        fallback_reason = (
                            f"recovery:{recovery_output.get('recovery_type')}"
                        )
                    else:
                        reply, no_result_meta_early = build_recovery_draft_message(
                            recovery_output,
                            filters={
                                **merged,
                                "category": requested_category,
                        ...
                        recovery_ctx = recovery_context_for_state(recovery_output, merged)
                        ...
                        turn_suggestions = recovery_output.get("suggestions") or []
                        save_conversation(session_id, user_message, reply)
                        return success_response(
                            message=reply,
                            data=_assistant_payload(
                                ...
                                search_status={
                                    ...
                                    "requested_category": requested_category,
                                    "fallback_used": False,
                                    "fallback_reason": recovery_output.get("recovery_type"),
                                },
                                context_extra={
                                    "response_goal": "explain_no_result_with_recovery",
                                    "no_result_recovery": recovery_ctx,
                                    "recovery_type": recovery_output.get("recovery_type"),
                                },
                            ),
                        )
    ...
    project_intro = ""
    no_result_meta: dict = {}
    if intent.get("intent") == "project_recommendation" and search_flags:
        pk = (search_flags or {}).get("project_type") or intent.get("project_type")
        if pk:
            project_intro = project_recommendation_explanation(pk, lang=reply_lang) + "\n\n"
    ...
    if exact_match_found:
        ...
        if not alternatives and not recovery_output:
            recovery_output = await _run_search_no_result_recovery(
                database,
                session_id=session_id,
                intent_name=intent.get("intent") or "machine_search",
            ...
            auto = recovery_output.get("auto_retry_results") or {}
            if recovery_output.get("auto_retried") and auto.get("machines"):
                alternatives = deduplicate_machines(without_embeddings(auto["machines"]))
                display = alternatives
                fallback_used = True
                fallback_reason = f"recovery:{recovery_output.get('recovery_type')}"

        if recovery_output:
            from app.ai.no_result_recovery import build_recovery_draft_message

            reply, no_result_meta = build_recovery_draft_message(
                recovery_output,
                filters={
                    **merged,
                    "category": requested_category,
    ...
    turn_suggestions = search_followup_suggestions(
        listing_type=listing_type,
        has_results=bool(display),
    )
    if not exact_match_found and requested_category:
        if recovery_output:
            turn_suggestions = recovery_output.get("suggestions") or no_result_suggestions(
                nearby_cities=no_result_meta.get("nearby_cities") if no_result_meta else [],
                similar_categories=similar_category_suggestions(requested_category or ""),
            )
        else:
            turn_suggestions = no_result_suggestions(
                nearby_cities=no_result_meta.get("nearby_cities") if no_result_meta else [],
                similar_categories=similar_category_suggestions(requested_category or ""),
            )
    ...
    if not exact_match_found:
        if recovery_output:
            response_goal = (
                "suggest_similar_machines" if alternatives else "explain_no_result_with_recovery"
            )
        else:
            response_goal = "suggest_similar_machines" if alternatives else "explain_no_result"

    response_variant_id = None
    from app.ai.conversation_aware_response import finalize_assistant_reply

    recovery_ctx_payload = None
    if recovery_output and not exact_match_found:
        from app.ai.no_result_recovery import recovery_context_for_state

        recovery_ctx_payload = recovery_context_for_state(recovery_output, merged)
        try:
            from app.ai.assistant_debug_trace import record_recovery
            record_recovery(recovery_output, recovery_state_saved=True)
        except Exception:
            pass

    final = await finalize_assistant_reply(
        draft_message=reply,
        intent=(classification or {}).get("llm_intent")
        or intent.get("intent")
        or "machine_search",
        assistant_mode=assistant_mode,
        response_goal=response_goal,
        entities={"name": session_ctx.get("user_name")},
        tool_result={
            "message": reply,
            "machines": display,
            "suggestions": turn_suggestions,
            ...
            "not_found_reason": (recovery_output or {}).get("message_facts", {}).get("not_found_reason"),
            "recovery_type": (recovery_output or {}).get("recovery_type"),
            "safe_alternatives": (recovery_output or {}).get("safe_alternatives"),
            "auto_retry_results": (recovery_output or {}).get("auto_retry_results"),
        },
        session_context={
            **session_ctx,
            "session_id": session_id,
    ...
    reply = final["message"]
    turn_suggestions = final.get("suggestions") or turn_suggestions
    response_variant_id = final.get("response_variant_id") or final.get("response_signature")
    ...
    return success_response(
        ...
        data=_assistant_payload(
            ...
            context_extra={
                ...
                "should_search_machines": True,
                "corrections": (spell_ctx or {}).get("corrections"),
                "response_goal": response_goal,
                "response_variant_id": response_variant_id,
                "no_result_recovery": recovery_ctx_payload,
                "recovery_type": (recovery_output or {}).get("recovery_type"),
            },
            input_meta=input_meta,
            suggestions=turn_suggestions,
