    ...
    *,
    ...
    if missing_field == "city":
        if label and label != "machine":
            return pick_lang(
                lang,
                english=(
                    f"Sure — {label} is a solid choice. Which city is your site in? "
                    "Share budget or rent/buy preference too and I'll shortlist the best matches."
                ),
                hindi=(
                    f"ठीक है — {label} अच्छा विकल्प है। प्रोजेक्ट किस शहर में है? "
                    "बजट या किराया/खरीद भी बताएँ, मैं सबसे अच्छे विकल्प दिखाऊँगा।"
                ),
                hinglish=(
                    f"Theek hai — {label} good choice hai. Kis city me chahiye? "
                    "Budget ya rent/buy bhi bata dein, main best matches dikhaunga."
                ),
            )
        return pick_lang(
            lang,
            english="Sure. Which city do you need the machine in?",
            hindi="ठीक है। आपको मशीन किस शहर में चाहिए?",
            hinglish="Theek hai. Kaunsi city me machine chahiye?",
        )
    if missing_field == "category":
        ...
        if city_title:
            return pick_lang(
                lang,
                english=f"Got it — which machine do you need in {city_title}?",
                hindi=f"ठीक है — {city_title} में आपको कौन सी मशीन चाहिए?",
                hinglish=f"Theek hai — {city_title} me kaunsi machine chahiye?",
            )
        return pick_lang(
            lang,
            english=(
                "Which machine type do you need? (e.g. excavator, JCB, crane, road roller)\n"
                "You can also share city, rent/buy preference, and budget."
            ),
            hindi=(
                "आपको कौन सी मशीन चाहिए? (जैसे excavator, JCB, crane, road roller)\n"
                "शहर, किराया/खरीद और बजट भी बता सकते हैं।"
            ),
            hinglish=(
                "Kaunsi machine chahiye? (e.g. excavator, JCB, crane, road roller)\n"
                "City, rent/buy aur budget bhi bata sakte hain."
            ),
        )
    return pick_lang(
        lang,
        english=(
            ...
            "• Machine type (excavator, crane, road roller…)\n"
            "• City\n"
            "• Rent or buy\n"
            "• Budget (optional)"
        ),
        ...
        hinglish=(
            "Main madad kar sakta hoon — batayein:\n"
            "• Kaunsi machine\n• City\n• Rent ya buy\n• Budget (optional)"
        ),
    )
...
def localized_no_exact_in_city(
    *,
    ...
    nearby_txt = format_city_list(nearby_cities)
    parts_en = [
        f"I couldn't find an exact {label} in {city_title} right now."
    ]
    if similar_txt:
        parts_en.append(f"Similar categories: {similar_txt}.")
    if nearby_txt:
        parts_en.append(
            f"Nearby options with listings: {nearby_txt}. "
            f"Would you like me to check {nearby_cities[0].title()}?"
        )
    parts_en.append(
