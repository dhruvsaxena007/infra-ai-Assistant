...
from app.ai.intent_classifier import NON_SEARCH_INTENTS  # canonical source
...
def build_response(
    ...
    *,
    ...
    knowledge = lookup_knowledge(message or "", intent=intent, lang=lang)
    if knowledge:
        return knowledge

    handover_msg = "What would you like to do?"
    if intent == "refund_return":
        ...
        return {
            ...
            "suggestions": support_action_suggestions("refund_return"),
            "handover": enrich_handover(
                build_handover("refund_return", message=handover_msg), "refund_return", lang=lang,
            ),
        }
    ...
    if intent == "order_issue":
        ...
        return {
            ...
            "suggestions": support_action_suggestions("order_issue"),
            "handover": enrich_handover(
                build_handover("order_issue", message=handover_msg), "order_issue", lang=lang,
            ),
        }
    ...
    if intent == "payment_issue":
        ...
        return {
            ...
            "suggestions": support_action_suggestions("payment_issue"),
            "handover": enrich_handover(
                build_handover("payment_issue", message=handover_msg), "payment_issue", lang=lang,
            ),
        }

    if intent == "invoice_issue":
        msg = (
            "Please share your booking/order ID so support can help with invoice or receipt."
            + oid_note
        )
        if lang == "hinglish":
            msg = "Invoice/receipt ke liye booking/order ID share karein." + oid_note
        return {
            "message": msg,
            "assistant_mode": "payment_issue",
            "suggestions": ["Share order ID", "Contact support", "Raise Request"],
            "handover": build_handover("invoice_issue", message=handover_msg),
        }
    ...
    if intent == "delivery_logistics":
        ...
        return {
            ...
            "suggestions": support_action_suggestions("delivery_logistics"),
            "handover": enrich_handover(
                build_handover("delivery_logistics", message=handover_msg),
                "delivery_logistics",
                lang=lang,
            ),
        }
    ...
    if intent == "support_request" or intent == "human_handover":
        msg = "Sure — I can connect you with support. Call, WhatsApp, or raise a request below."
        if lang == "hinglish":
            msg = "Theek hai — main aapko support se connect kar sakta hoon. Neeche Call, WhatsApp ya Raise Request use karein."
        return {
            "message": msg,
            "assistant_mode": "support",
            "suggestions": [],
    ...
    if intent == "general_marketplace_help" or intent == "general_help":
        msg = (
    ...
    if intent == "frustration":
        msg = (
            "I'm sorry this hasn't been helpful. Tell me the machine type and city in one message, "
            "or say support for booking or payment help."
        )
        if lang == "hinglish":
            msg = (
                "Sorry for the trouble. Ek message me machine type aur city bata dein, "
                "ya booking/payment ke liye support likhein."
            )
        return {
            "message": msg,
            "assistant_mode": "support",
            "suggestions": ["Excavator in Jaipur", "Booking issue", "Contact support"],
            "handover": None,
        }
    ...
    if intent == "unknown":
        ...
        if lang == "hinglish":
            msg = (
                "Mujhe poora clear nahi hai kaunsi machine chahiye. "
                "Kaam ka type aur city bata dein?"
            )
        return {
            "message": msg,
