...
This module is the single source of truth for:
  - category synonym mapping (explicit aliases + model-implied fallback)
  - brand detection (via brand_catalog — brands never override explicit categories)
  - city detection
...
#
# IMPORTANT: Brand names (JCB, CAT, Hyundai, …) are NOT category synonyms.
# Model lines (3DX, 4DX) imply backhoe loader only when no explicit category is present.
# Matching is performed longest-synonym-first so multi-word terms win.
# ---------------------------------------------------------------------------
...
CATEGORY_SYNONYMS = {
    ...
    "excavator": [
        "excavator", "excavators", "earth excavator", "hydraulic excavator",
    ...
    "bulldozer": [
        "bulldozer", "bulldozers", "dozer", "crawler dozer",
    ],
    "road roller": [
        "road roller", "roadroller", "road-roller", "roadrollers", "road-rollers",
        "roller", "steamroller", "vibratory roller", "tandem roller",
        "soil compactor", "compactor roller", "road compactor", "roller compactor",
        "sadak roller", "sadak ka roller", "road rolling", "compaction roller",
    ],
    "drum roller": [
    ...
    "wheel loader": [
        "front end loader", "wheel loader", "wheel-loader", "wheelloader",
        "front loader",
    ],
    "crawler drill": [
    ...
    "concrete pump": [
        "concrete pump", "concrete-pump", "boom pump", "trailer pump", "schwing stetter",
    ],
    "air compressor": [
...
MODEL_CATEGORY_SYNONYMS = {
    "backhoe loader": [
        "jcb 3dx", "jcb 4dx", "jcb 3 dx", "jcb 4 dx", "3dx", "4dx",
    ],
}
...
_MODEL_SYNONYM_LOOKUP = sorted(
    (
        (synonym, canonical)
        for canonical, synonyms in MODEL_CATEGORY_SYNONYMS.items()
        for synonym in synonyms
    ),
    key=lambda pair: len(pair[0]),
    reverse=True,
...
# Ambiguous single-token category aliases — only match with word boundaries
# when not part of a longer explicit synonym already matched.
_AMBIGUOUS_CATEGORY_TOKENS = frozenset({
    "roller", "loader", "mixer", "crane", "drill", "compactor", "breaker",
})
...
def _preprocess_category_text(text: str) -> str:
    """Normalize joined/hyphenated category tokens before synonym matching."""
    if not text:
        return ""
    out = text.lower()
    replacements = (
        (r"\broadrollers?\b", "road roller"),
        (r"\broad-rollers?\b", "road roller"),
        (r"\bwheelloaders?\b", "wheel loader"),
        (r"\bwheel-loaders?\b", "wheel loader"),
        (r"\bconcretepumps?\b", "concrete pump"),
        (r"\bconcrete-pumps?\b", "concrete pump"),
        (r"\bdumptrucks?\b", "dump truck"),
        (r"\bdump-trucks?\b", "dump truck"),
        (r"\bmotorgraders?\b", "motor grader"),
        (r"\bmotor-graders?\b", "motor grader"),
        (r"\bbackhoeloaders?\b", "backhoe loader"),
        (r"\bbackhoe-loaders?\b", "backhoe loader"),
    )
...
def _synonym_matches(synonym: str, text: str, *, ambiguous_ok: bool = True) -> bool:
    if not synonym or not text:
        return False
    if synonym in _AMBIGUOUS_CATEGORY_TOKENS and not ambiguous_ok:
        return False
    return _word_in_text(synonym, text)


def _match_categories_from_lookup(
    text: str,
    lookup: list[tuple[str, str]],
    *,
    allow_ambiguous: bool = True,
) -> list[str]:
    """Return canonical categories found in text (first-seen order, de-duplicated)."""
    if not text:
        return []
    lowered = _preprocess_category_text(text)
    found: list[str] = []
    matched_spans: list[tuple[int, int]] = []

    for synonym, canonical in lookup:
        if canonical in found:
            continue
        if synonym in _AMBIGUOUS_CATEGORY_TOKENS and not allow_ambiguous:
            continue
        pattern = re.compile(rf"\b{re.escape(synonym)}\b", re.I)
...
def _detect_explicit_categories(text: str) -> list[str]:
    return _match_categories_from_lookup(text, _SYNONYM_LOOKUP, allow_ambiguous=True)


def _detect_model_implied_category(text: str) -> Optional[str]:
    """Model/product-line category fallback when no explicit category is present."""
    if not text:
        return None
    lowered = _preprocess_category_text(text)
    for synonym, canonical in _MODEL_SYNONYM_LOOKUP:
...
def detect_category(text: str):
    ...
    Priority:
      1. Explicit category / alias in query
      2. Model-specific phrase (e.g. 3DX -> backhoe loader)
    Brand names alone never set category.
    """
    if not text:
        return None

    explicit = _detect_explicit_categories(text)
    if explicit:
        return explicit[0]

    return _detect_model_implied_category(text)
...
_PRICE_PATTERNS = [
    r"under\s*(?:rs\.?|inr|₹)?\s*([\d,]+(?:\.\d+)?)\s*(k)?",
    r"below\s*(?:rs\.?|inr|₹)?\s*([\d,]+(?:\.\d+)?)\s*(k)?",
    r"less\s+than\s*(?:rs\.?|inr|₹)?\s*([\d,]+(?:\.\d+)?)\s*(k)?",
    r"max(?:imum)?\s*(?:rs\.?|inr|₹)?\s*([\d,]+(?:\.\d+)?)\s*(k)?",
    r"budget\s*(?:of)?\s*(?:rs\.?|inr|₹)?\s*([\d,]+(?:\.\d+)?)\s*(k)?",
    r"within\s*(?:rs\.?|inr|₹)?\s*([\d,]+(?:\.\d+)?)\s*(k)?",
    r"up\s*to\s*(?:rs\.?|inr|₹)?\s*([\d,]+(?:\.\d+)?)\s*(k)?",
    r"upto\s*(?:rs\.?|inr|₹)?\s*([\d,]+(?:\.\d+)?)\s*(k)?",
]
...
def detect_max_price(text: str):
    ...
    expanded = expand_numeric_shorthand(text)
    amount = extract_budget_amount(expanded)
...
def _all_categories_present(text: str) -> list:
    """All explicit canonical categories mentioned in text, de-duplicated, first-seen order."""
    explicit = _detect_explicit_categories(text)
    if explicit:
        return explicit
    model_cat = _detect_model_implied_category(text)
    return [model_cat] if model_cat else []
...
_MARKETPLACE_CATEGORY_PATTERNS = [
    ...
    ("backhoe loader", ["backhoe", "loader backhoe", "3dx", "4dx"]),
    ...
    ("road roller", ["road roller", "roadroller", "road-roller", "steamroller"]),
...
def detect_entity_validation_notes(text: str, parsed: dict) -> list[str]:
    """Conflict flags for entity extraction (e.g. model vs explicit category)."""
    notes: list[str] = []
    if not text:
        return notes
    explicit = _detect_explicit_categories(text)
    ...
    if model_cat and explicit and model_cat != explicit[0]:
        notes.append("model_category_conflict")
    elif model and category and model_cat and model_cat != category:
        notes.append("model_category_conflict")
    return notes
...
def detect_brand(text: str):
    ...
    from app.ai.brand_catalog import detect_brand as _detect_brand_catalog
    return _detect_brand_catalog(text)


def detect_all_brands(text: str) -> list[str]:
    """Detect all brands mentioned in query text."""
    from app.ai.brand_catalog import detect_all_brands as _detect_all
    return _detect_all(text)


# Backward-compatible exports used elsewhere in the codebase.
KNOWN_BRANDS = [
    "epiroc", "jcb", "caterpillar", "cat", "tata", "volvo", "komatsu",
    "hyundai", "hitachi", "tata hitachi", "schwing stetter", "atlas copco",
    "ammann apollo", "terex", "sany", "liebherr", "case", "mahindra", "ace",
    "kobelco", "liugong", "escorts",
]
...
def detect_pincode(text: str):
    if not text:
        return None
    lowered = text.lower()
    # Budget/rate context — 6-digit values are amounts, not postal codes
    if re.search(
        r"\b(?:budget|under|below|max|upto|lakh|lac|crore|per\s+day|rent|kiraye)\b",
        lowered,
    ):
        return None
