...
def build_chat_input_metadata(
    *,
    original_message: str,
    normalized_message: str,
    working_message: str | None = None,
) -> dict:
    """Attach once per turn — rules-only, no duplicate LLM parse."""
    msg = (working_message or normalized_message or original_message or "").strip()
    understanding = understand_user_text_rules(msg)
    data = understanding.get("data") if understanding.get("success") else {}
    return {
        "original_message": (original_message or "").strip(),
        "normalized_message": (normalized_message or msg).strip(),
        "enhanced_query": (data or {}).get("translated_query") or msg,
        "text_understanding": data or None,
    }
