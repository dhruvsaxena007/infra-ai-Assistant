...
async def build_multi_purpose_advisory(
    ...
    *,
    ...
    if lang == "hinglish":
        intro = f"Aapke project ke liye ye machines suitable hain{city_note}:\n\n"
        if not city:
            intro = "Aapke kaam ke liye ye machine types suitable hain:\n\n"
        msg = intro + "\n".join(f"• {line}" for line in lines)
        if all_machines:
            msg += f"\n\n{city.title() if city else 'City'} me kuch live options:"
        elif city:
            msg += "\n\nExact match limited ho sakta hai — nearby city try karein ya niche se category pick karein."
        else:
            msg += "\n\nKaunsi city me machine chahiye? City bataiye, main listings dhoondhunga."
    else:
        intro = f"For your project, these machine types are commonly used{city_note}:\n\n"
        if not city:
            intro = "For your work, these machine types fit best:\n\n"
        msg = intro + "\n".join(f"• {line}" for line in lines)
        if all_machines:
            msg += f"\n\nHere are some live listings{city_note}:"
        elif city:
            msg += "\n\nLimited exact matches — try a nearby city or pick a category below."
        else:
