...
_add("human_handover", r"human\s+handover|real\s+person|agent\s+se\s+baat|insaan\s+se\s+baat|call\s+support", 0.91)
...
_add("payment_issue", r"payment\s+(?:failed|fail|stuck|pending|issue)|payment\s+cut|amount\s+deducted|paisa\s+kat|double\s+charge|transaction\s+(?:failed|issue)|payment\s+ho\s+gaya.*(?:confirm|booking).*(?:nahi|not)|booking\s+confirm\s+nahi\s+hui|booking\s+nahi\s+hui", 0.9)
...
_add("invoice_issue", r"\binvoice\b|\breceipt\b|bill\s+chahiye|gst\s+invoice|tax\s+invoice", 0.87)
_add("security_deposit", r"security\s+deposit|deposit\s+(?:issue|refund|policy|kitna|amount)", 0.88)
_add("order_issue", r"(?:ordered|booked|booking).{0,60}(?:problem|issue|help|wrong|delay)|(?:problem|issue).{0,40}(?:order|booking)|meri\s+booking.*issue|machine\s+(?:nahi\s+aayi|abhi\s+tak\s+nahi|not\s+delivered|wrong\s+deliver\w*)|machine\s+abhi\s+tak.*(?:deliver|aayi|nahi)|abhi\s+tak\s+nahi\s+aayi|wrong\s+machine\s+deliver\w*|rental\s+start\s+nahi", 0.9)
...
_add("general_help", r"\bhelp\b|\bguide\b|what\s+do\s+you\s+support", 0.8)
_add("document_question", r"document|pdf|attachment.*question|uploaded\s+(?:pdf|document)", 0.8)
_add("document_question", r"what\s+(?:does|do)\s+(?:the\s+)?(?:document|pdf|file|attachment)", 0.85)
...
_add("compare_machine", r"\b(?:better|best|prefer)\b.{0,80}\b(?:or|ya|aur)\b", 0.91)
_add("compare_machine", r"\b(?:jcb|cat|caterpillar)\b.{0,50}\b(?:or|vs|versus|ya)\b.{0,50}\b(?:jcb|cat|caterpillar|road\s+roller|excavator)\b", 0.92)
...
def _result(
    *,
    ...
    should_search: Optional[bool] = None,
...
def _compute_should_search(intent: str, entities: dict, parsed: dict, message: str) -> bool:
    if intent in ("frustration", "acknowledgement", "out_of_scope", "compare_machine", "machine_comparison"):
        return False
    if intent == "machine_brand_query":
        return bool(entities.get("machine_type") or parsed.get("category"))
    if intent == "higher_budget_query" or intent == "cheaper_option_query":
        return bool(entities.get("machine_type") or entities.get("city") or parsed.get("category"))
...
def _requires_clarification(intent: str, entities: dict, parsed: dict, message: str) -> bool:
    ...
    if intent == "broad_machine_request":
        return True
    if intent == "machine_recommendation":
        return not _compute_should_search(intent, entities, parsed, message)
    if intent == "machine_brand_query":
        return not bool(entities.get("machine_type") or parsed.get("category"))
    if intent == "higher_budget_query" or intent == "cheaper_option_query":
        return not _compute_should_search(intent, entities, parsed, message)
...
def _requires_handover(intent: str) -> bool:
    return intent in {
        "order_issue", "refund_return", "payment_issue", "invoice_issue",
        "complaint", "delivery_logistics", "security_deposit",
        "support_request", "human_handover",
    }
...
def _rule_classify(message: str, context: Optional[dict] = None) -> dict[str, Any]:
    ...
    # Frustration / recovery — never search
    if is_frustration_signal(combined):
        return _result(
            intent="frustration",
            confidence=0.9,
            entities=entities,
    ...
    # Acknowledgement within active flow
    if is_acknowledgement_signal(combined, ctx):
        return _result(
    ...
    if _is_business_priority(combined):
        business = [
            "support_request", "human_handover", "refund_return", "payment_issue",
            "invoice_issue", "security_deposit", "order_issue", "complaint",
            "delivery_logistics",
        ]
        best_biz = max(
    ...
    # Machine comparison (structural — uses parser entities)
    if is_machine_comparison_signal(combined, parsed):
        entities = enrich_entities_from_context(entities, parsed, ctx)
        return _result(
            ...
            message=combined,
            normalized=normalized,
            reason="comparison_signal",
            should_search=False,
            requires_clarification=not bool(entities.get("machine_type") or len(entities.get("brands") or []) >= 2),
        )

    # Brand / inventory query
    if is_machine_brand_query_signal(combined, parsed):
        entities = enrich_entities_from_context(entities, parsed, ctx)
        return _result(
            ...
            message=combined,
            normalized=normalized,
            reason="brand_inventory_signal",
        )

    # Session follow-up: higher budget / cheaper refinement
    last = ctx.get("last_filters") or {}
    ctx_with_msg = {**(ctx or {}), "message": combined}
    if is_higher_budget_signal(combined, ctx_with_msg, parsed):
        entities = enrich_entities_from_context(entities, parsed, ctx)
        return _result(
            intent="higher_budget_query",
            confidence=0.86,
            entities=entities,
            parsed=parsed,
            message=combined,
            normalized=normalized,
            reason="higher_budget_follow_up",
        )

    if is_cheaper_option_signal(combined, ctx_with_msg, parsed):
        entities = enrich_entities_from_context(entities, parsed, ctx)
        return _result(
            ...
            message=combined,
            normalized=normalized,
            reason="cheaper_refinement_follow_up",
        )

    # Broad machine request (rent/buy/vague — no category assumed)
    if is_broad_machine_request_signal(combined, parsed):
        return _result(
            ...
            message=combined,
            normalized=normalized,
            reason="broad_machine_signal",
            should_search=False,
            requires_clarification=True,
        )

    # Machine recommendation — clarify when work/city missing
    if is_recommendation_broad_signal(combined, parsed) or (
        is_recommendation_query(combined) and "machine_recommendation" in scores
    ):
        entities = enrich_entities_from_context(entities, parsed, ctx)
        should = _compute_should_search("machine_recommendation", entities, parsed, combined)
        return _result(
            intent="machine_recommendation",
            confidence=scores.get("machine_recommendation", 0.84),
            entities=entities,
            parsed=parsed,
            message=combined,
            normalized=normalized,
            reason="recommendation_broad" if not should else "recommendation_query",
            should_search=should,
            requires_clarification=not should,
        )
    ...
    if last.get("category") or last.get("city"):
        if re.search(
            r"\b(?:same\s+in|cheaper|what\s+about|show\s+more|under\s+\d|sasta|"
            r"higher\s+budget|premium|expensive|rent\s+only|buy\s+only)\b",
            combined, re.I,
        ) or is_higher_budget_signal(combined, ctx_with_msg, parsed) or is_cheaper_option_signal(
            combined, ctx_with_msg, parsed,
        ):
            entities = enrich_entities_from_context(entities, parsed, ctx)
            intent = "higher_budget_query" if is_higher_budget_signal(combined, ctx_with_msg, parsed) else (
                "cheaper_option_query" if is_cheaper_option_signal(combined, ctx_with_msg, parsed) else "machine_search"
            )
            return _result(
                intent=intent,
                confidence=0.8,
        ...
        if parsed.get("city") and not parsed.get("category") and last.get("category") and not last.get("city"):
            ...
            return {
                ...
                "requires_handover": False,
                "entities": entities,
                "reason": "city_only_follow_up_incomplete_prior",
                "layer": "rules",
                "normalized_message": normalized,
            }

    # Vague machine query → broad_machine_request (not unknown)
    if is_broad_vague_query(combined) or is_broad_machine_request_signal(combined, parsed):
        return _result(
            intent="broad_machine_request",
            confidence=0.72,
            entities=entities,
...
async def classify_intent(
    ...
    When AI_INTENT_MODE=llm_first, LLM is primary with rules fallback.
    When AI_INTENT_MODE=llm_shadow, rules route but LLM is logged for comparison.
    """
    if settings.llm_shadow:
        from app.ai.llm_intent_classifier import classify_intent_llm_shadow_compare
        try:
            from app.ai.assistant_debug_trace import record_llm_intent_called, record_rules_intent_called
            record_llm_intent_called()
            record_rules_intent_called()
        except Exception:
            pass
        rules_result, llm_result = await classify_intent_llm_shadow_compare(message, context=context)
        try:
            from app.ai.assistant_debug_trace import record_intent_shadow_comparison
            record_intent_shadow_comparison(rules_result, llm_result)
        except Exception:
            pass
        rules_result["intent_source"] = "rules_shadow"
        return rules_result

    if settings.llm_intent_first:
        from app.ai.llm_intent_classifier import classify_intent_llm
        try:
            from app.ai.assistant_debug_trace import record_llm_intent_called
            record_llm_intent_called()
        except Exception:
            pass
        ...
        try:
            from app.ai.assistant_debug_trace import record_intent_resolution
            source = result.get("intent_source") or ("llm" if result.get("layer") == "llm" else "rules_fallback")
            record_intent_resolution(result, final_source=source)
        except Exception:
            pass
        return result

    try:
        from app.ai.assistant_debug_trace import record_rules_intent_called
        record_rules_intent_called()
    except Exception:
        pass
    ...
    if result["intent"] in {
        "refund_return", "order_issue", "payment_issue", "invoice_issue",
        "complaint", "delivery_logistics", "security_deposit", "support_request",
        "human_handover", "platform_how_to", "general_marketplace_help",
        "general_help", "out_of_scope", "unknown", "document_question",
        "booking_help", "greeting", "contact_owner", "compare_machine",
        "machine_comparison", "frustration", "acknowledgement", "broad_machine_request",
    }:
...
def is_machine_search_intent(intent: str) -> bool:
    """True when router may invoke execute_machine_search_turn."""
    return intent in SEARCH_INTENTS or intent == "machine_recommendation"


def is_blocked_search_intent(intent: str) -> bool:
    """Intents that must never trigger machine search."""
    if intent in ("machine_brand_query", "higher_budget_query", "cheaper_option_query"):
        return False
